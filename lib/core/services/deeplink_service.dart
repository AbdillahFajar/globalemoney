...
class DeeplinkPaymentData {
  final String merchantId; //Id unik merchant, tidak boleh kosong
  final String merchantName; //Nama merchant (URL-decoded otomatis)
  final double amount; //Nominal Rupiah > 0
  final String description; //Keterangan; default "Pembayaran ke <merchantName>"
  final String? reference; //Nomor referensi/invoice
  final String? callbackUrl; //URL callback (disimpan, tidak auto redirect)
...
class DeeplinkService {
  ...
  Future<void> init() async {
    ...
    // Cold-start: simpan URI sebagai pending (tangkap link yang membuka app), JANGAN navigasi sekarang.
    ...
    // In-app: tangkap link saat app berjalan dan navigasi via post-frame setelah GoRouter ter-mount.
  ...
  void _handleInAppUri(Uri uri) {
    ...
    try {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        _router.go('/pay', extra: data); //valid -> halaman konfirmasi
      });
    } on FormatException catch (e) {
      debugPrint('[DeeplinkService] Format error: ${e.message}');
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _router.go('/pay', extra: e.message); //error -> halaman error
      });
    }
  }
